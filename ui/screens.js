...
const SCREEN_IDS = ["today", "history", "session", "done", "run", "workout", "weights", "bodycomp"];
...
const TAB_SCREENS = ["today", "workout", "weights", "history", "bodycomp"];
...
// active: "today" | "workout" | "weights" | null (напр. на экране
// «История» — она не вкладка, подсветки нет).
export function renderTabbar(active) {
  for (const t of ["today", "workout", "weights"]) {
...
export function getWeightsApiKey() { return $("weights-key-input").value.trim(); }
export function openWeightsSettings() { $("weights-settings").open = true; }
export function closeWeightsSettings() {
  $("weights-key-input").value = "";
  $("weights-key-input").placeholder = "Ключ сохранён — ввести новый?";
  $("weights-settings").open = false;
}
