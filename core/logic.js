...
export function pullupDayScheme(programNumber, week, day, maxReps) {
  ...
  if (day === "T") return `Тест: разминка, затем 1 (один) подход на максимум строгих (текущий макс ${maxReps})`;
  ...
  const table = {
    1: { A: `5×${work} (макс ${maxReps} − 2), отдых 2-3 мин`, C: `4×${work}, пауза 2 с вверху, отдых 2-3 мин`, B: ladder(2) },
    2: { A: `5×${work} (макс ${maxReps} − 2), отдых 2-3 мин`, C: `5×${work}, отдых 2-3 мин`, B: ladder(3) },
    3: { A: `6×${work} (макс ${maxReps} − 2), отдых 2-3 мин`, C: "перетест: 1 подход на максимум строгих, потом 3 подхода по (новый макс − 2)", B: "лесенка от нового макса, 2 круга, между кругами 3 мин" },
    4: { A: `3×${easy}, легко (разгрузка)`, C: `2×${easy}, легко`, B: "пятница — день замеров: 1 (один) подход на максимум строгих" },
  };
